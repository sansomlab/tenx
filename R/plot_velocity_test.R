...
option_list <- list(
    ...
    make_option(c("--minmaxclustavmmat"), type="double",
                default=0.02,
...
names(cmap) <- as.character(c(0:(nclust-1)))
...
drawVelocityPlot <- function()
{
    ...
    cell_mass=1
    show.velocity.on.embedding.cor(as.matrix(emb),rvel.cd,n=200,scale='log',
                                   cell.colors=ac(cell.colors,alpha=0.8),
                                   cex=1,arrow.scale=3,show.grid.flow=T,
                                   min.grid.cell.mass=cell_mass,grid.n=40,arrow.lwd=1.25,
                                                                  do.par=F,cell.border.alpha = 0.1,axes=F)
    }
